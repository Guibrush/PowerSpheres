...
#include "PowerSpheres.h"

static void ToMergeParams(const TArray<FPSSkelMeshMergeSectionMapping_BP>& InSectionMappings, TArray<FSkelMeshMergeSectionMapping>& OutSectionMappings)
{
...
static void ToMergeParams(const TArray<FPSSkelMeshMergeUVTransformMapping>& InUVTransformsPerMesh, TArray<FSkelMeshMergeUVTransforms>& OutUVTransformsPerMesh)
{
	if (InUVTransformsPerMesh.Num() > 0)
	{
		...
		for (int32 i = 0; i < InUVTransformsPerMesh.Num(); ++i)
		{
			...
			const TArray<FPSSkelMeshMergeUVTransform>& InUVTransforms = InUVTransformsPerMesh[i].UVTransformsPerMesh;
...
USkeletalMesh* UPSMeshMergeFunctionLibrary::MergeMeshes(const FPSSkeletalMeshMergeParams& Params)
{
	...
	{
		UE_LOG(LogPowerSpheres, Warning, TEXT("Must provide multiple valid Skeletal Meshes in order to perform a merge."));
		return nullptr;
	}
	...
	if (Params.Skeleton && Params.bSkeletonBefore)
	{
		...
		for (USkeletalMeshSocket* Socket : BaseMesh->GetMeshOnlySocketList())
		{
			if (Socket)
			{
				UE_LOG(LogPowerSpheres, Warning, TEXT("SkelMeshSocket: %s"), *(Socket->SocketName.ToString()));
			}
		}
		for (USkeletalMeshSocket* Socket : BaseMesh->Skeleton->Sockets)
		{
			if (Socket)
			{
				UE_LOG(LogPowerSpheres, Warning, TEXT("SkelSocket: %s"), *(Socket->SocketName.ToString()));
			}
		}
	}
	...
	if (!Merger.DoMerge())
	{
		UE_LOG(LogPowerSpheres, Warning, TEXT("Merge failed!"));
		return nullptr;
	}
	...
	if (bRunDuplicateCheck)
	{
		...
		UE_LOG(LogPowerSpheres, Warning, TEXT("SkelMeshSocketCount: %d | SkelSocketCount: %d | Combined: %d"), SkelMeshSockets.Num(), SkelSockets.Num(), Total);
		UE_LOG(LogPowerSpheres, Warning, TEXT("SkelMeshSocketCount: %d | SkelSocketCount: %d | Combined: %d"), UniqueSkelMeshSockets.Num(), UniqueSkelSockets.Num(), UniqueTotal);
		UE_LOG(LogPowerSpheres, Warning, TEXT("Found Duplicates: %s"), *((Total != UniqueTotal) ? FString("True") : FString("False")));
	}
